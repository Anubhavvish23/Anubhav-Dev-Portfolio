...
const ProficiencyCard: React.FC<ProficiencyCardProps> = ({ name, icon, progress }) => {
  return (
    <div className="proficiency-card">
      <svg className="proficiency-wave" viewBox="0 0 1440 320" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="proficiency-icon-container">
        {typeof icon === 'string' ? (
          <span className="proficiency-icon-emoji">{icon}</span>
        ) : (
          icon
        )}
      </div>
      <div className="proficiency-text-container">
        <p className="proficiency-name">{name}</p>
        <p className="proficiency-sub">{progress}% proficiency</p>
      </div>
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" strokeWidth={0} fill="currentColor" stroke="currentColor" className="proficiency-check-icon">
